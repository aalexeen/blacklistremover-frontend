    ...
    <section class="bg-blue-50 px-4 py-10">
        <div class="container-xl lg:container m-auto">
            ...
            <div v-if="error" class="bg-white rounded-lg shadow p-6 text-center text-red-500">
                <p class="mb-4">Error loading MAC addresses. Please try again.</p>
                <button 
                    @click="fetchMacAddresses(true)" 
            ...
            <div v-else-if="isLoading" class="bg-white rounded-lg shadow p-8 text-center text-gray-500">
                <PulseLoader />
                <p class="mt-4">Loading MAC addresses...</p>
            </div>

            <!-- Show mac listing table when done loading -->
            <div v-else-if="displayedMacs.length > 0" class="bg-white rounded-lg shadow overflow-hidden">
            ...
            <div v-else class="bg-white rounded-lg shadow p-8 text-center text-gray-500">
                <p v-if="hasActiveSearch">No MAC addresses found matching your search: "{{ searchQuery }}"</p>
